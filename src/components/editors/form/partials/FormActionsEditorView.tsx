...
const FormActionsEditorView =  ({form: {getFieldDecorator, validateFieldsAndScroll}}) => {
    ...
    return useObserver(() => {
        return form && <Form {...formItemLayout} onSubmit={(e) => localStore.handleSubmit(e)} layout={"horizontal"}>
                <Form.Item label="Success Redirect URL" help="Users will be redirected here on successful submission.">
                    {
                        getFieldDecorator('successRedirect', {
                            initialValue: store.formStore.form.successRedirect,
                            rules: [{pattern: /^(https?):\/\/[^\s$.?#].[^\s]*$/gm}]
                        })(<Input />)
                    }
                </Form.Item>
                <Form.Item label="Error Redirect URL (Optional)" help="Users will be redirected here on submission failure.">
                    {
                        getFieldDecorator('errorRedirect', {
                            initialValue: store.formStore.form.errorRedirect,
                            rules: [{pattern: /^(https?):\/\/[^\s$.?#].[^\s]*$/gm}]
                        })(<Input />)
                    }
                </Form.Item>
                <Form.Item label="Success Message" help="Shown on successful submissions (if no redirect)">
                    {
                        getFieldDecorator('submitSuccessMessage', {
                            initialValue: store.formStore.form.submitSuccessMessage,
                        })(<Input.TextArea />)
                    }
                </Form.Item>
                <Form.Item label="Error Message" help="Shown on submission failure (if no redirect)">
                {
                    getFieldDecorator('submitErrorMessage', {
